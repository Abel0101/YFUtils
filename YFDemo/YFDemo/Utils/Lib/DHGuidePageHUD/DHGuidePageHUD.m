    ...
    if ([super initWithFrame:frame]) {
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                [startButton setTitle:NSLocalizedString(@"Start" , @"开始") forState:UIControlStateNormal];
...
//                [startButton setCornerRadius:5];
        ...
        self.imagePageControl.hidden = self.isHidePageController;
...
- (void)setIsHidePageController:(BOOL)isHidePageController{
    self.imagePageControl.hidden = isHidePageController;
    _isHidePageController = isHidePageController;
}

- (void)setIsHideSkipBtn:(BOOL)isHideSkipBtn{
    self.skipBtn.hidden = isHideSkipBtn;
    _isHideSkipBtn = isHideSkipBtn;
}

...
//    NSString *mobielNumberRegex = @"^\\d+$";
    ...
    NSPredicate *mobielNumber = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", phoneRegex];
    return [mobielNumber evaluateWithObject:self];
}

- (BOOL)validateMobileNumberOfCountryCode:(NSString *)countryCode {
    NSString *mobielNumberRegex;
    if ([countryCode isEqualToString:@"s_chinaCode"]) {
        mobielNumberRegex = @"^\\d{11}$";
...
+ (NSString *)getLetter:(NSString *) strInput{
    ...
    if ([strInput length]) {
        ...
        if ([ms isEqualToString:@"sha men"]) {
            ms = [NSMutableString stringWithFormat:@"%@", @"xia men"];
        }
